...
public class Pintar extends Applet implements KeyListener, Runnable{
    ...
    private static final int VELOCIDAD_ENEMIGOS = 50;
    ...
    String mensaje = "";
    Color color = null;
    int posiscionTextoX = 0,posiscionTextoY = 0;
    ...
    private void limpiarPantalla(Graphics g) {
        ...
        if (enemigos.isEmpty()) { 
            Sonido.sounds[3].play();
            setColor(Color.BLUE); 
            setMensaje("WINNER!"); 
            setPosiscionTextoX(200);
            setPosiscionTextoY(250); 
        }else{
            Sonido.sounds[2].play();
            setMensaje("GAME OVER!"); 
            setColor(Color.BLACK); 
            setPosiscionTextoX(150);
            setPosiscionTextoY(250); 
        }
        ...
        g.drawString(getMensaje(), getPosiscionTextoX(), getPosiscionTextoY()); 
        g.setFont(new Font("TimesRoman", Font.PLAIN, 40)); 
        g.drawString("Puntucion: "+getPuntaje(), 250, 330); 
        g.drawString("Vidas: "+getVidas(), 250, 380); 
        g.drawString("Enemigos Vivos: "+enemigos.size(), 250, 420); 
         
         
        g.setFont(new Font("TimesRoman", Font.PLAIN, 30)); 
        g.drawString("Grupo: Nocturno || Semestre IX", 210, 500);  
        g.drawString("Bryan Blanco || Jesus Salcedo", 210, 540);
        g.drawString("Corporación Unificada Nacional de Educación Superior ( CUN )", 10, 580);
        g.drawString("Ingeniería de Sistemas || 2020A", 210, 620);
    ...
    public void run() {
        while(running){
            ...
            esperar(500);
            repaint();
            if(getVidas() == 0 || enemigos.isEmpty()){
                stop();
                repaint();
            }
        }
    ...
    public int getPosiscionTextoX() {
        return posiscionTextoX;
    }

    public void setPosiscionTextoX(int posiscionTextoX) {
        this.posiscionTextoX = posiscionTextoX;
    }

    public int getPosiscionTextoY() {
        return posiscionTextoY;
    }

    public void setPosiscionTextoY(int posiscionTextoY) {
        this.posiscionTextoY = posiscionTextoY;
    }
